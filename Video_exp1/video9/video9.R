...
X <- long_grid(x = seq(-3, 3, length.out = 500), 
          y = seq(-3, 3, length.out = 500)) %>% 
    mutate(
        x1 = x + gen_perlin(x, y, seed = rpois(n = 1, lambda = 5) + 1) / 4 - gen_simplex(x, y, seed = rpois(n = 1, lambda = 5) + 1) / 4, 
        y1 = y - gen_waves(x, y) / 4 + gen_cubic(x, y, seed = rpois(n = 1, lambda = 5) + 1) / 4,
        worley = gen_worley(x, y, value = 'distance2', frequency = 5, seed = rpois(n = 1, lambda = 5) + 1),
        worley_frac = fracture(gen_cubic, billow, octaves = 30, x = x, y = y, seed = rpois(n = 1, lambda = 5) + 1),
        full = blend(normalise(worley), normalise(worley_frac), gen_waves(x1, y1))
    )
X <- matrix(X$full, ncol = 500, byrow = TRUE)

Y <- long_grid(x = seq(-3, 3, length.out = 500), 
          y = seq(-3, 3, length.out = 500)) %>% 
    mutate(
        x1 = x + gen_perlin(x, y, seed = rpois(n = 1, lambda = 5) + 1) / 4 - gen_simplex(x, y, seed = rpois(n = 1, lambda = 5) + 1) / 4, 
        y1 = y - gen_waves(x, y) / 4 + gen_cubic(x, y, seed = rpois(n = 1, lambda = 5) + 1) / 4,
...
Y <- matrix(Y$full, ncol = 500, byrow = TRUE)

colores <- scico(n = 50, palette = "tokyo")
