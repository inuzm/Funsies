...
chale2 <- function(x, y){
    (
        c4 * dnorm(-sinpi(x)^2 + y, mean = d4, sd = e4) + 
        c5 * dnorm(cospi(sinpi(x^2 + y^2)^3), mean = d5, sd = e5) + 
...
chale3 <- function(x, y){
    (
        c7 * dnorm(cospi(x^2 * y^2) - sinpi(-x^2 + y^2), mean = d7, sd = e8) + 
        c8 * dnorm(exp(cospi(sinpi(-x^2 + y^2))), mean = d8, sd = e9) + 
        c9 * dnorm(((x^3 + y^3) %% 1.1) * sign(x^3 + y^3), mean = d9, sd = e9)
    )
}

chale4 <- function(x, y){
    (
        c10 * dnorm(cospi(exp(x^3)) - sinpi(y)^3, mean = d10, sd = e10) + 
        c11 * dnorm(sinpi(x^3) * cos(y^3), mean = d11, sd = e11) + 
        c12 * dnorm(((-x^3 + y^3) %% 2.3) * sign(-x^3 + y^3), mean = d12, sd = e12)
    )
}
...
x1 <- seq(from = -0.5, to = 0.5, length = 300)
y1 <- seq(from = -0.5, to = 0.5, length = 300)
...
convex <- 0
convex_max <- 0.5
delta <- seq(from = 1, to = 10, length = nimages)
...
for(k in 1:nimages){
    ...
    par(mar = c(0,0,0,0) + 0.1)
    if(convex <= 0.2){
        image(x = y, y = y, z = outer(x, y, chale_simple), col = scico(n = 50, palette = "oleron", alpha = 1 - (5*convex)))
        image(x = y, y = y, z = outer(x, y, chale1), col = magma(n = 50, alpha = (5 * convex)), add = TRUE)
    } else if (convex <= 0.4) {
        image(x = y, y = y, z = outer(x, y, chale1), col = magma(n = 50, alpha = 2 - (5 * convex)))
        image(x = y, y = y, z = outer(x, y, chale2), col = plasma(n = 50, alpha = (5 * convex) - 1), add = TRUE)
    } else if (convex <= 0.6) {
        image(x = y, y = y, z = outer(x, y, chale2), col = plasma(n = 50, alpha = 3 - (5 * convex)))
        image(x = y, y = y, z = outer(x, y, chale3), col = alpha(ghibli_palette(name = "PonyoMedium", n = 50, type = "continuous"), alpha = (5 * convex) - 2), add = TRUE)
    } else if (convex <= 0.8) {
        image(x = y, y = y, z = outer(x, y, chale3), col = alpha(ghibli_palette(name = "PonyoMedium", n = 50, type = "continuous"), alpha = 4 - (5 * convex)))
        image(x = y, y = y, z = outer(x, y, chale4), col = alpha(wes_palette("FantasticFox1", n = 50, type = "continuous"), alpha = (5 * convex) - 3), add = TRUE)
    } else {
        image(x = y, y = y, z = outer(x, y, chale4), col = alpha(wes_palette("FantasticFox1", n = 50, type = "continuous"), alpha = 5 - (5 * convex)))
        image(x = y, y = y, z = outer(x, y, chale5), col = alpha(colorRampPalette(carto_pal(7, "Vivid"))(50), alpha = (5 * convex) - 4), add = TRUE)
    }
    dev.off()
