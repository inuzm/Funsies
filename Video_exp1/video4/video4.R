...
chale <- function(x, y){
    (
        (c1 * x^e1 + c2 *  y^e2) *
            exp(sign( cos(c3 * x^e3) * sin(c4 * y^e4) )) *
            sin(sqrt(c5* x^e5 + c6 * y^e6)) *
            cos(c7 * x^e7 + c8 * y^e8)
    ) %% 50.47
}
...
for(k in 1:nimages){
    ...
    contour(x = x, y = y, z = outer(x, y, mixture(convex)), drawlabels = FALSE, col = "black", nlevels = 4)
    dev.off()

    png(paste0(output_directory, sprintf("/%04d_col.png", k)), width = 600, height = 600, res = 75)
    par(mar = c(0,0,0,0) + 0.1)
    image(x = y, y = y, z = outer(x, y, mixture(convex)), col = ghibli_palette(name = "MononokeMedium", n = 30, type = "continuous"))
    dev.off()
    ...
    c5 <- max(c5 + rnorm(n = 1, sd = sqrt(1e-3)), 0)
    c6 <- max(c6 + rnorm(n = 1, sd = sqrt(1e-3)), 0)
